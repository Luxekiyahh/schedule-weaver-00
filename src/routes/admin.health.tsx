  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card title="Stripe payment failures" icon={<CreditCard className="h-4 w-4 text-primary" />}>
          {data.stripe.error && <div className="mb-2 text-xs text-amber-600">{data.stripe.error}</div>}
          <div className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Past-due subscriptions ({data.stripe.pastDueSubscriptions.length})</div>
          ...
          {data.stripe.failedCharges.map((c: any) => (
            <div key={c.id} className="py-1 text-xs text-muted-foreground">
        ...
        <Card title="Twilio delivery failures" icon={<MessageSquare className="h-4 w-4 text-primary" />}>
          ...
          {data.twilio.failed.map((m: any) => (
            <div key={m.sid} className="py-1 text-xs text-muted-foreground">
              {m.to} · <span className="text-rose-600">{m.status}</span>{m.error_code ? ` (${m.error_code})` : ""}
            </div>
          ))}
          <div className="mb-2 mt-3 text-xs font-semibold uppercase text-muted-foreground">Logged send failures ({data.twilio.recentFailedLogs.length})</div>
        ...
        <Card title="Email failures" icon={<Mail className="h-4 w-4 text-primary" />}>
          {data.email.recentFailures.length === 0 && <Empty text="No recent failures." />}
          {data.email.recentFailures.map((e: any) => (
            <div key={e.id} className="py-1 text-xs text-muted-foreground">
        ...
        >
          ...
          {data.cron.map((j: any) => (
            <div key={j.jobid} className="border-b border-border py-1.5 text-xs last:border-0">
              <div className="flex justify-between">
                <span className="font-medium text-foreground">{j.jobname}</span>
                <span className={j.last_status === "succeeded" ? "text-emerald-600" : j.last_status ? "text-rose-600" : "text-muted-foreground"}>
...
function Empty({ text }: { text: string }) {
  return <div className="py-1 text-sm text-muted-foreground">{text}</div>;
}
