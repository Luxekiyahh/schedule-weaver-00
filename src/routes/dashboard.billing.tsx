      ...
      <div className="max-w-5xl mx-auto p-6 space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                Current plan
                {currentTier ? (
                  <Badge className="bg-foreground text-background">{planByTier(currentTier).name}</Badge>
                ) : (
        ...
        <div className="flex justify-center">
          <div className="inline-flex items-center rounded-full border border-border bg-card p-1 text-sm font-medium">
            <button
              ...
              className={`rounded-full px-4 py-1.5 transition ${period === "monthly" ? "bg-foreground text-background" : "text-muted-foreground"}`}
            >
              Monthly
            </button>
            <button
              ...
              className={`rounded-full px-4 py-1.5 transition ${period === "yearly" ? "bg-foreground text-background" : "text-muted-foreground"}`}
            >
              Annual
              <span className={`ml-1.5 text-xs ${period === "yearly" ? "text-background/60" : "text-emerald-600"}`}>2 months free</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {PLANS.map((plan) => {
            ...
            return (
              <Card key={plan.tier} className={isCurrent ? "border-foreground ring-1 ring-foreground" : plan.tier === "pro" ? "border-foreground/20" : ""}>
                <CardHeader>
                  <div className="flex items-center justify-between">
          ...
          <Card className="border-foreground/20 bg-foreground/[0.04]">
            <CardContent className="pt-6 md:flex md:items-center md:justify-between md:gap-6">
              <div className="flex items-start gap-3">
                <div className="rounded-xl bg-foreground p-2.5 text-background shrink-0">
