        ...
        const handler = createEmailWebhookHandler({
          ...
          on: {
            'email.bounced': async (event) => {
              await recordOutcome(event.data.recipient, 'bounce', event.event_id)
            },
            'email.complaint': async (event) => {
              await recordOutcome(event.data.recipient, 'complaint', event.event_id)
            },
            'email.unsubscribed': async (event) => {
              await recordOutcome(event.data.recipient, 'unsubscribe', event.event_id)
            },
          },
        })
